...
export class WhatsappSessionNoWebCore extends WhatsappSession {
  ...
  async sendVideo(request: MessageVideoRequest) {
    ...
    if (request.asNote) {
      // Round/instant video note. Baileys uses ptv for this.
      message.ptv = true;
    }
    const options: any = await this.getMessageOptions(request);
    return this.sock.sendMessage(chatId, message, options);
  ...
  async sendLinkCustomPreview(
    ...
    const chatId = toJID(this.ensureSuffix(request.chatId));
    const jpegThumbnail = await this.uploadMedia(
      request.preview.image as any,
      'image',
    );
    const linkPreview: any = {
      'canonical-url': request.preview.url,
      'matched-text': request.preview.url,
      title: request.preview.title,
      description: request.preview.description,
    };
    if (jpegThumbnail) {
      linkPreview.jpegThumbnail = jpegThumbnail;
    }
    const message: any = {
      text: request.text,
  ...
  protected async setNewsletterPicture(
    jid: string,
    buffer: any,
  ): Promise<void> {
    // Newsletters use a dedicated IQ namespace; generic updateProfilePicture
    // is ignored by the server for newsletter JIDs. Prefer the newsletter-
    // specific API when the Baileys fork exposes it, fall back to the
    // generic call otherwise so older Baileys versions still behave as before.
    ...
    try {
      ...
      this.logger.warn(
        { jid: jid, err: err?.message },
        'Failed to set newsletter picture',
      );
    }
  }
  ...
  protected async uploadMedia(
    ...
    if ('url' in file) {
      // Download URL and convert to Buffer
      const response = await fetch(file.url);
      if (!response.ok) {
        throw new Error(
          `Failed to download media from ${file.url}: ${response.statusText}`,
        );
      }
    ...
    // Handle BinaryFile (data)
    if ('data' in file) {
      // Convert base64 or binary data to Buffer
      let buffer: Buffer;
  ...
  @Activity()
  public async channelsGetChannelMessages(
    id: string,
    query: GetChannelMessagesQuery,
  ): Promise<ChannelMessage[]> {
    const jid = toJID(id);
    const sock: any = this.sock;
    if (typeof sock.newsletterFetchPreviewMessages !== 'function') {
      throw new Error(
        'Baileys socket does not expose newsletterFetchPreviewMessages',
      );
    }
    const count = query.limit ?? 50;
    const after = query.after;
    const raw = await sock.newsletterFetchPreviewMessages(
      'jid',
      jid,
      count,
      after,
    );
    const updates: any[] = Array.isArray(raw) ? raw : [];
    const promises = updates.map((upd) =>
      this.toChannelMessageFromUpdate(upd, query.downloadMedia),
    );
    const results = await Promise.all(promises);
    return results.filter((m): m is ChannelMessage => m !== null);
  }

  protected async toChannelMessageFromUpdate(
    upd: any,
    ...
    const rawReactions: any[] = Array.isArray(upd.reactions)
      ? upd.reactions
      : [];
    for (const r of rawReactions) {
      if (r?.code && typeof r.count === 'number') {
        reactions[r.code] = r.count;
      }
    }
    return {
      message: waMessage,
      reactions: reactions,
      viewCount: typeof upd.views === 'number' ? upd.views : 0,
    };
  }
  ...
  public async channelsCreateChannel(request: CreateChannelRequest) {
    ...
    if (request.picture) {
      const buffer = await this.uploadMedia(request.picture, 'image');
      if (buffer) {
        await this.setNewsletterPicture(newsletter.id, buffer);
        const refreshed = await this.sock.newsletterMetadata(
          'jid',
          newsletter.id,
        );
        const metadata = toNewsletterMetadata(refreshed);
        if (metadata) {
          return this.toChannel(metadata);
        }
      }
    }
  ...
  @Activity()
  public async channelsUpdateChannel(
    id: string,
    ...
    if (request.name !== undefined) {
      if (typeof sock.newsletterUpdateName === 'function') {
        await sock.newsletterUpdateName(id, request.name);
      } else {
        this.logger.warn(
          { jid: id },
          'Baileys socket does not expose newsletterUpdateName',
        );
      }
    }
    if (request.description !== undefined) {
      if (typeof sock.newsletterUpdateDescription === 'function') {
        await sock.newsletterUpdateDescription(id, request.description);
      } else {
        this.logger.warn(
          { jid: id },
          'Baileys socket does not expose newsletterUpdateDescription',
        );
      }
    }
    if (request.picture) {
      const buffer = await this.uploadMedia(request.picture, 'image');
      if (buffer) {
        await this.setNewsletterPicture(id, buffer);
      }
    }
    const refreshed = await this.sock.newsletterMetadata('jid', id);
    const metadata = toNewsletterMetadata(refreshed);
    if (!metadata) {
      throw new Error(`Channel ${id} not found after update`);
    }
    return this.toChannel(metadata);
  }
